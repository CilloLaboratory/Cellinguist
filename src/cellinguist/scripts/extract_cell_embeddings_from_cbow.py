...
def compute_cell_embeddings(
    ...
    token_embeddings : torch.Tensor
        ...
        Must correspond to dataset.token_to_id / dataset.id_to_token.
    ...
    for i, token_ids_np in enumerate(dataset._token_id_seqs):
        # token_ids_np is a 1D np.ndarray of token IDs for cell i
        if token_ids_np.size == 0:
            ...
            # Safety check: ensure IDs are within vocab range
            if token_ids_np.max() >= vocab_size or token_ids_np.min() < 0:
                raise ValueError(
                    f"Token IDs out of range for cell {i}. "
                    f"Got min={token_ids_np.min()}, max={token_ids_np.max()}, "
...
def main():
    ...
    parser.add_argument(
        "--embeddings",
        required=True,
        help="Path to learned CBOW token embeddings (e.g. gene_embeddings.pt).",
    )
    parser.add_argument(
        "--out",
        required=True,
    ...
    parser.add_argument(
        "--no-shuffle-tokens",
        action="store_true",
        help="Disable per-cell token shuffling (must match CBOW training if you change this).",
    )
    ...
    ds = SingleCellDataset(
        ...
        token_to_id=None,         # rebuild vocab from this dataset
        min_token_count=1,
    )

    # Sanity check: vocab size == embedding rows
    ...
    cell_embs = compute_cell_embeddings(ds, token_embeddings)  # (n_cells, emb_dim)

    # 4. Build DataFrame with cell IDs
    # Use adata.obs_names as cell identifiers if available
