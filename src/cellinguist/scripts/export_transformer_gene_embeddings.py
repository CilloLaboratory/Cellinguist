...
def export_transformer_gene_embeddings(
    checkpoint_path: str,
    out_tsv_gz: str,
) -> None:
    ...
    if weight is None:
        raise ValueError(
            "Could not find transformer gene embedding weights in checkpoint state_dict."
        )
    ...
    print(f"[export] Wrote Transformer gene embeddings to: {out_path}")


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Export Transformer gene token embeddings from a VAE checkpoint."
    )
    ...
    export_transformer_gene_embeddings(
